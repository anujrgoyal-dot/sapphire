    ...
    <div className="page-content">
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>Tap "New Order" to create your first sales order</p>
        </div>
      ) : (
        filtered.map(order => (
          <OrderCard
            ...
            onEdit={() => navigate(`/edit-order/${order.id}`)}
            onDownload={() => generateSaleOrderPDF(order)}
          />
        ))
      )}
    </div>
...
function OrderCard({ order, statusColor, onEdit, onDownload }) {
  ...
  return (
    <div className="order-card">
      <div className="order-card-header">
        ...
        <span className={`badge ${statusColor[order.status] || 'badge-draft'}`}>
          {order.status}
        </span>
      </div>
