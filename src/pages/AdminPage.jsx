              ...
              <div className="card-body">
                <div className="flex-between mb-2">
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{new Date(order.so_date).toLocaleDateString('en-IN')} · {order.salesperson_name}</div>
                  </div>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4 }}>
                  {(order.items || []).length} items · Total Qty: {Number(order.total_qty || 0).toFixed(0)}
                </div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  {['draft', 'submitted'].includes(order.status) && (
                    <button className="btn btn-ghost btn-sm" onClick={() => updateStatus(order.id, 'draft')}>↩ To Draft</button>
                  )}
                </div>
              </div>
            </div>
...
function AdminClients() {
  ...
  const [form, setForm] = useState({
    customer_code: '', name: '', address: '', city: '', state: 'Maharashtra',
    pincode: '', gst_no: '', pan_no: '', phone: '', contact_person: '', email: ''
  })
  ...
  function openNew() {
    setForm({ customer_code: '', name: '', address: '', city: '', state: 'Maharashtra', pincode: '', gst_no: '', pan_no: '', phone: '', contact_person: '', email: '' })
    setEditing(null)
    setShowForm(true)
  }
            ...
            <div className="flex-between">
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                  {c.customer_code} · {c.city} · {c.phone}
                </div>
...
function AdminInventory() {
  ...
  async function handleExcelUpload(e) {
    ...
    try {
      ...
      // Clear existing SKUs and re-insert in batches
    ...
    <div>
      <div className="card mb-3">
        <div className="card-body">
          ...
          <div style={{ fontSize: 32, fontWeight: 800, color: 'var(--navy)' }}>
            {skuCount !== null ? skuCount.toLocaleString('en-IN') : '...'}
          </div>
      ...
      <div className="card">
        <div className="card-header">
          <div className="card-title">Update Inventory / MRP</div>
        </div>
        <div className="card-body">
          <div className="alert alert-warning mb-3">
            ⚠️ Uploading a new Excel will REPLACE all existing SKUs. Make sure your file has columns: Item Description, UOM, Tax Rate, HSN Code, MRP
          </div>
          ...
          <div style={{ border: '2px dashed var(--border)', borderRadius: 10, padding: 24, textAlign: 'center', marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginBottom: 12 }}>
              .xlsx file with your inventory & MRP
            </div>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>
            <strong>Expected columns:</strong> A=Item Description, B=UOM, C=Tax Rate (0.18), D=HSN Code, E=MRP
          </div>
        </div>
      </div>
    </div>
...
function AdminUsers() {
  ...
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({ email: '', name: '', password: '', role: 'salesperson' })
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const [success, setSuccess] = useState('')
  ...
  async function createUser() {
    setSaving(true)
    setError('')
    // Note: Creating users requires admin privileges via Supabase dashboard or Edge Function
    // Here we guide the admin to use Supabase dashboard
    setSuccess('To create a user: Go to Supabase Dashboard → Authentication → Users → Add User. Then add their UUID to the profiles table with their name and role.')
    setSaving(false)
  }
